...
public class JPanelMovie extends JPanel {
	
//	//当前图片序号
//	int step = 1;
//	Image temp = null;
//	
//	public JPanelAboutUs(JFrameGame game) {
//		this.game = game;
//		// 设置为绝对布局
//		this.setLayout(null);
//	}
...
//	//读取图片
	...
	public static Image getAMimage(int m){
		Image img = null;
		try {
			if(m>=0&&m<=9){
				img = ImageIO.read(new File("image/aboutus/Dreamer&&Doer_0000" + m + ".png"));
			}else if(m>=10&&m<=99){
				img = ImageIO.read(new File("image/aboutus/Dreamer&&Doer_000" + m + ".png"));
			}else if(m>=100&&m<=999){
				img = ImageIO.read(new File("image/aboutus/Dreamer&&Doer_00" + m + ".png"));
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
